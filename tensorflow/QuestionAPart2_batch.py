...
logging.basicConfig(level=logging.INFO, filename="output_batch", filemode="a+",
...
def calc_aggregator(j):
    tf.scatter_sub(w, tf.gather(indices_list,j),tf.gather(gradients, j))
    j = tf.add(j, 1)
    return j
...
def _parse_function(example_proto):
    ...
    return features['index'], features['label'], features['value']
...
with g.as_default():
    ...
    for i in range(0, 5):
        with tf.device("/job:worker/task:%d" % i):
            ...
            dataset = tf.data.TFRecordDataset(file_distributions[i])
            dataset = datatset.map(_parse_function)
            datatset = dataset.batch(BATCH_SIZE)
            iterator = datatset.make_initializable_iterator()
