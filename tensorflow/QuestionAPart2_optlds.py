...
def calc_aggregator(j0):
    j0 += 1
    #print tf.gather(indices_list, j0)
#    print indices_list[1].get_shape()
#    print gradients[1].get_shape()
    #print tf.gather(gradients, j0)
    tf.scatter_sub(w, tf.gather(indices_list,j0),tf.gather(gradients, j0))
    return j0
...
with g.as_default():
    ...
    for i in range(0, 5):
	    ...
	    # dense_feature = tf.gather(value.values, value.indices)
...
#                                           tf.constant([33762578, 1], dtype=tf.float32),
	    ...
	    #print "gradients now", len(gradients)
            indices_list.append(indices)
	    #print index
	    #print "indices_list now", len(indices_list)
	    #ind = tf.constant(0)
	    #print tf.gather(indices_list, ind)


    # we create an operator to aggregate the local gradients
    with tf.device("/job:worker/task:0"):
        # aggregator = tf.add_n(gradients)
        # aggregator = tf.scatter_add()
        # assign_op = tf.assign_sub(w, aggregator)
        j = tf.constant(-1)
#        agg = tf.Variable(tf.ones([num_features,]), name="aggr", dtype=tf.float32)
	#print "indices list ",indices_list
	#print "gradients list ", gradients
        c = lambda j0 : j0 < 5
        #b = lambda j0,agg : [ j0+1, tf.scatter_sub(w, tf.gather(indices_list, [j0]), tf.gather(gradients, [j0]))]
        assign_op = tf.while_loop(c, calc_aggregator, [j])

	#c = [lambda : i for gradient in gradients]
