...
#	print total_gain
	...
	# if gain is 0, allot minimum of 10 cores
	    if total_gain and cpu_allot_dict[job.id]!=0:
                ...
                print "The allocation based on eq: ", self.allocation[job.id]
            job.allotted_cpus = self.allocation[job.id]
	    print "accuracy of the job %s is %f" % (job.name, job.accuracy) 
        return cpu_allot_dict
   
    def bimodal_allocation_resources(self,jobs,is_first=False):
        number_of_jobs = len(jobs)
        cpu_allot_dict={}
        total_gain = 0
        total_weight = 0
        ...
        for job in jobs:
            ...
            if found == False:
                ...
                continue
            if gain >= 0.001 :  
                cpu_allot_dict[job.id] = 2
                total_weight += 2
            else: 
                cpu_allot_dict[job.id] = 1
                total_weight += 1
            #print "gain value for job %s is %f"%(job.name, gain)
        ...
        #print "weights so far: ", cpu_allot_dict
#       print total_gain
        total_cpu = self.total_cpu
        #for job in jobs:
        #    if cpu_allot_dict[job.id]<=0 :
        ...
        # if gain is 0, allot minimum of 10 cores
            if total_gain and cpu_allot_dict[job.id] != 0:
                self.allocation[job.id]= (cpu_allot_dict[job.id]*total_cpu)/ total_weight
                print "Entering loop: ", self.allocation[job.id]
            job.allotted_cpus = self.allocation[job.id]
            print "accuracy of the job %s is %f" % (job.name, job.accuracy)
        print "Allocations looks like: ", self.allocation
        return cpu_allot_dict
	...
	#		is_first=True
		...
		for i in range(maxlimit):
            ...
            print "Running bimodal scheduler"
            for i in range(maxlimit):
                self.bimodal_allocation_resources(jobs, is_first = not i)
...
if __name__ == '__main__':
    ...
    for i in range(12, 15):
        job = Job(i, name[i - 12])
        binomial_jobs.append(job)
    driver.start_jobs(binomial_jobs, "bimodal")
